...
GLuint LoadShadersCS(const char * computer_file_path){
	...
	std::ifstream ComputerShaderStream(computer_file_path, std::ios::in);
	if(ComputerShaderStream.is_open()){
		std::string Line = "";
		while(getline(ComputerShaderStream, Line))
			ComputerShaderCode += "\n" + Line;
		ComputerShaderStream.close();
	}else{
		printf("Impossible to open %s. Are you in the right directory ? Don't forget to read the FAQ !\n", computer_file_path);
		return 0;
	}
