...
class phonglighting_app : public OpenGLApp
{
public:
	phonglighting_app()
...
void phonglighting_app::load_shaders()
{
	if (per_fragment_program)
		glDeleteProgram(per_fragment_program);
	per_fragment_program = LoadShadersVF("../media/glsl/12_phonglighting/per-fragment-phong.vs", "../media/glsl/12_phonglighting/per-fragment-phong.fs");
	...
	if (per_vertex_program)
		glDeleteProgram(per_vertex_program);
	per_vertex_program = LoadShadersVF("../media/glsl/12_phonglighting/per-vertex-phong.vs", "../media/glsl/12_phonglighting/per-vertex-phong.fs");
...
void phonglighting_app::render(double currentTime)
{
    ...
    for (j = 0; j < 7; j++)
    {
        for (i = 0; i < 7; i++)
        {
            ...
            glUniform3fv(uniforms[per_vertex ? 1 : 0].specular_albedo, 1, glm::value_ptr(glm::vec3((float)i / 9.0f + 1.0f / 9.0f)));
    ...
    glUniform3fv(uniforms[per_vertex ? 1 : 0].specular_albedo, 1, glm::value_ptr(glm::vec3(1.0f)));

    object.render();
#endif
}

void phonglighting_app::onKey(int key, int scancode, int action, int mods)
{
